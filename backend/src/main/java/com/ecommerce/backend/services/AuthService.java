...
public class AuthService {
    ...
    public ResponseEntity<?> tryLogin(LoginRequest request) {

        // UseLogger.info("--- INICIO PROCESO LOGIN ---", "");
        // UseLogger.info("Email recibido: ", request.getEmail());
        // UseLogger.info("Password recibida (longitud): ",
        // String.valueOf(request.getPassword().length()));
        ...
        try {
            ...
            if (debugUser.isPresent()) {
                ...
                // UseLogger.info("¿COINCIDENCIA MANUAL BCrypt?: ", matches ? "SÍ (TRUE)" : "NO
                // (FALSE)");
                ...
                // UseLogger.info("Si generamos un hash ahora para '" + request.getPassword() +
                // "' sería: ",
                // nuevoHashTest);
            }
            // ---------------------------------------
    ...
    public ResponseEntity<LoginResponse> register(WebUser user) {
